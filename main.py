...
def fetch_youtube_transcript(video_url):
    """Fetches transcript for a YouTube video if available."""
    try:
        ...
        # Try to get transcript (prefer English, then auto-generated)
        ...
        if transcript:
            ...
            return full_text[:8000]  # Limit to save tokens
    except Exception as e:
        print(f"⚠️ Could not fetch transcript: {e}")
    return None

def summarize_youtube_video(video_info, transcript=None):
    """Summarizes a YouTube video based on title, description, and optionally transcript."""

    if transcript:
        system_prompt = """
        You are an expert tech editor for a bilingual blog covering YouTube tech videos.
        You have access to the video transcript for accurate summarization.
        1. Analyze the video transcript and information.
        2. Output a detailed summary in English and Chinese.
        3. STRICTLY use Markdown formatting.
        4. Highlight: main points covered, key insights, why it's worth watching.
        5. Structure:
           ### 🎬 [Video Title]
           **Channel:** [Channel Name]
           * Main topic and thesis
           * Key points and insights (3-5 bullets)
...
if __name__ == "__main__":
    ...
    for video in channel_videos:
        ...
        if transcript:
            print(f"📝 Found transcript for: {video['title']}")
        summary = summarize_youtube_video(video, transcript)
